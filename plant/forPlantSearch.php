...
<?php
 ...
 $plantNameK = $_POST['plantNameK'];
 $plantNameE = $_POST['plantNameE'];


 $connect = mysql_connect('localhost','signi2','Aa0a829**');
 mysql_select_db('signi2');

     $sql = "select * from `flower` where urlName = '$plantNameE' OR name = '$plantNameK' ";   //DB에서 아이디값과 비밀번호가 동일한
 ...
 //    $result = mysql_query($con,$sql);
   ...
   //  $count=mysql_num_rows($result);                                                                 //몇개의 데이터가 반환되었는지 확인
     if($count>=1) {
        ...
        echo "<meta http-equiv='refresh' content='0; url=./plantSearch.php'>";
      }
?>
